...
public:
  ...
  Sound(const unsigned char* raw_pcm, std::size_t size, std::size_t frequency, Format format);
  Sound(std::vector<unsigned char> raw_pcm, std::size_t frequency, Format format);
  Sound(Sound&& mv) = default;
  ~Sound();
  auto play(SoundPlayInfo info) -> void;
  auto stop() -> void;
private:
  struct SoundData;
